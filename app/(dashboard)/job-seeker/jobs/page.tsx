...
import { useJobs, useBookmarkJob, useRemoveBookmark, useCheckBookmark } from '@/lib/hooks/use-jobs';
...
export default function JobsPage() {
  ...
  return (
    // CHANGE 1: Changed from fixed height to min-h-screen to allow window scrolling
    <div className="min-h-screen"> 
      ...
      {/* Split View */}
      {/* CHANGE 2: Added items-start to prevent stretching and removed fixed height */}
      <div className="grid lg:grid-cols-[500px_1fr] items-start gap-6 p-6 pb-0">
        
        {/* Job List (Left Side) */}
        {/* CHANGE 3: Removed ScrollArea wrapper so it scrolls with the window */}
        ...
        {/* CHANGE 4: Added sticky positioning logic here */}
        <div className="hidden lg:block sticky top-17 h-[calc(100vh-3rem)]">
          {selectedJob ? (
             <div className="h-full border bg-card rounded-lg overflow-hidden shadow-sm">
...
// Job Card Component (Unchanged logic, just ensure it renders cleanly)
function JobCard({
  job,
  isSelected,
  onClick,
...
function JobDetails({ job }: { job: any }) {
  ...
  return (
    <div className="flex flex-col">
        {/* Keep ScrollArea here: This allows scrolling inside the sticky card */}
        <ScrollArea className="">
        <motion.div
            key={job.id} // Added key to trigger animation on job change
